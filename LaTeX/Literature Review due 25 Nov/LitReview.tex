...
\pagebreak
    ...
    % Restructure, and talk about Vectorisation! 
    % Vectorisation absolutely key. Word2Vec, Seq2Vec, etc. Perhaps RAG's Vector DB talk can go here too?
    % Finish by becoming increasingly specific to the applications of NLP for an LLM-based RAG chatbot.
    ...
    % TALK ABOUT EMBEDDING MODELS. MASSIVE KEY POINT FOR CHATBOT DEVELOPMENT.
    ...
    % Watson Assistant, MS Bot Framework, etc.
    ...
    % Pinecone is not free, nor is it open source. Just because one guy in the history of ever has used it doesn't mean you have to.
    % Open-source ones exist. ChromaDB. Additionally, you WILL be using Langchain I'm almost certain.
    ...
    % Likely requires a full rewrite given that everything's being altered.
